...
除了 `基础远程 (3)`、`基础近战 (4)` 和 `未使用 (31)` 之外的所有类型的伤害都会在乘以 $(1 - 0.33)$ 后才加和到西西里单位受到的总伤害中。这个附加伤害抗性属性由单位的[附加伤害抗性](../attributes/attributes/#24-bonus-damage-resistance "跳转至：游戏机制 > 属性 > 附加伤害抗性")属性控制。（对，每个单位都可以有不同的值）

实际上，并非所有西西里的单位都享有此抗性，而是只有下表中列出的类型的单位才有。
